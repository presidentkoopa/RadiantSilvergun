// RS_GH_SMG -- GunstarHeroes set, imported from the source arsenal pack.
// ---------------------------------------------------------------------
// Taken from the source: sprites (HBSM + SMGR), the model and its real frame
// indices (see MODELDEF -- copied verbatim from the pack's own
// modeldef.vr_hb_weapons, nothing inferred), the fire sound, and the
// weapon's SHAPE: slot 4, ballistic, 1 pellet(s), 3.5 degree cone,
...
class RS_GH_SMG : RS_Weapon
{
	Default
	{
		...
		Weapon.AmmoType2 "RS_GH_SMGLoaded";
		Inventory.Icon "HBSMA0";
		+WEAPON.NOHANDSWITCH;
	}
	...
	// midpoint, with tiers ramping from it, so a GH SMG is a rolled weapon
	...
	override void RollStats(EVR_Tier t)
	{
		...
		switch (t)
		{
			...
			case VRT_Cursed:
				...
				Accuracy      = RS_Roll.RollDouble(55, 67);
				Velocity      = RS_Roll.RollDouble(6500, 9000);
	...
	Ready:
		HBSM A 1 A_WeaponReady(WRF_ALLOWRELOAD);
		Loop;
	...
	Reload:
		...
		SMGR ABCDEFGHIJKLMNOPQRST 1;
		TNT1 A 0 A_RS_ReloadAtomic();
		Goto Ready;

	Flash:
		TNT1 A 0 A_RS_MuzzleFlash();
		HBSM B 1 Bright A_Light2();
		Stop;
...
class RS_GH_SMG2 : RS_GH_SMG
{
	Default
	{
		Tag "Gunstar SMG II";
		Weapon.SelectionOrder 1599;
		Weapon.AmmoType2 "RS_GH_SMGLoaded2";
	}
}

class RS_GH_SMG3 : RS_GH_SMG
{
	Default
	{
		Tag "Gunstar SMG III";
		Weapon.SelectionOrder 1598;
		Weapon.AmmoType2 "RS_GH_SMGLoaded3";
	}
}

class RS_GH_SMG4 : RS_GH_SMG
{
	Default
	{
		Tag "Gunstar SMG IV";
		Weapon.SelectionOrder 1597;
		Weapon.AmmoType2 "RS_GH_SMGLoaded4";
		+WEAPON.OFFHANDWEAPON;
	}
}

class RS_GH_SMG5 : RS_GH_SMG
{
	Default
	{
		Tag "Gunstar SMG V";
		Weapon.SelectionOrder 1596;
		Weapon.AmmoType2 "RS_GH_SMGLoaded5";
		+WEAPON.OFFHANDWEAPON;
	}
}

class RS_GH_SMG6 : RS_GH_SMG
{
	Default
	{
		Tag "Gunstar SMG VI";
		Weapon.SelectionOrder 1595;
		Weapon.AmmoType2 "RS_GH_SMGLoaded6";
		+WEAPON.OFFHANDWEAPON;
	}
}
