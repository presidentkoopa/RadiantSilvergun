...
class RS_HiFiFX
{
	...
	static void MuzzleEffects(Actor shooter, bool heavyFire = false)
	{
		int tier = Tier();
		if (tier == RSFX_OFF)
			return;
	...
	static void CasingEject(Actor shooter, string casingClass, double xoff = 4.0, double zoff = 0.0)
	{
		if (Tier() == RSFX_OFF)
			return;
	...
	static void MagDrop(Actor shooter, string dropClass)
	{
		if (Tier() != RSFX_HIFI)
			return;
		if (!dropClass.Length())
	...
	const MAX_CONCURRENT_MUZZLE_LIGHTS = 12;

	// Dynamic muzzle light -- Hi-Fi tier only; this is the expensive one.
	static void SpawnMuzzleLight(Actor shooter)
	{
		if (Tier() != RSFX_HIFI)
			return;
		...
		{
			count++;
			if (count >= MAX_CONCURRENT_MUZZLE_LIGHTS)
