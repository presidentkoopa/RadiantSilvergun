...
{
	Default { Radius 12; Height 12; Speed 14; FastSpeed 19; /* CH: Damage (random(10,40))  Fatsos.txt:2147 -- was flattened to `Damage 25`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(10,40)); DamageType "Plasma"; Projectile; +RANDOMIZE; +SEEKERMISSILE; RenderStyle "Add"; Alpha 1; Scale 0.65;
...
{
	Default { Radius 7; Height 7; Speed 28; /* CH: Damage (random(20,85))  Fatsos.txt:646 -- was flattened to `Damage 50`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,85)); DamageType "Ice"; Projectile; +USEBOUNCESTATE; +BOUNCEONWALLS; BounceFactor 1.1; BounceCount 3; WallBounceFactor 1.1;
...
{
	Default { Radius 20; Height 16; Speed 25; /* CH: Damage (random(14,60))  Fatsos.txt:694 -- was flattened to `Damage 35`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(14,60)); DamageType "Plasma"; Projectile; +MTHRUSPECIES; +DONTHARMCLASS; RenderStyle "Add"; Alpha 1; XScale 1.0; YScale 0.5;
...
{
	Default { Radius 3; Height 3; Speed 45; /* CH: Damage (random(10,20))  Fatsos.txt:942 -- was flattened to `Damage 15`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(10,20)); DamageType "Plasma"; Projectile; RenderStyle "Add"; Alpha 0.95; Scale 0.33;
...
{
	Default { Radius 20; Height 20; Mass 600; Speed 8; /* CH: Damage (random(15,50))  Fatsos.txt:913 -- was flattened to `Damage 30`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(15,50)); DamageType "Plasma"; Projectile; Scale 1.5; RenderStyle "Add"; Alpha 0.95;
...
{
	Default { Radius 7; Height 6; Speed 42; /* CH: Damage (random(20,80))  Fatsos.txt:2528 -- was flattened to `Damage 50`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,80)); DamageType "Fire"; Projectile; RenderStyle "Add"; Alpha 1; Scale 1.25;
		SeeSound "fatso/attack"; DeathSound "fatso/shotx"; }
	States { Spawn: MANF AB 3 Bright; Loop; Death: MISL BCD 4 Bright A_Explode(50,80); Stop; }
}
class RS_ShadowBeast_Ball1 : Actor
{
	Default { Alpha 1.0; RenderStyle "Add"; Speed 15; Radius 10; Height 6; /* CH: Damage (random(20,50))  Fatsos.txt:3367 -- was flattened to `Damage 35`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,50)); DamageType "Poison"; Projectile; SeeSound "shadowbeast/pr1sit"; DeathSound "shadowbeast/pr1death";
...
class RS_ShadowBeast_Ball3 : RS_ShadowBeast_Ball1 { Default { Scale 1.4; Radius 8; Speed 20; /* CH: Damage (random(20,60))  Fatsos.txt:3418 -- was flattened to `Damage 40`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,60)); DamageType "Plasma"; } }
...
{
	Default { Alpha 1.0; RenderStyle "Add"; Speed 8; Radius 14; Height 9; /* CH: Damage (random(20,90))  Fatsos.txt:3101 -- was flattened to `Damage 50`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,90)); Scale 1.8; DamageType "Plasma"; Projectile;
...
{
	Default { Radius 9; Height 9; Speed 21; /* CH: Damage (random(20,50))  Fatsos.txt:3922 -- was flattened to `Damage 35`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,50)); DamageType "Fire"; Projectile; Scale 1.5; +DONTHARMCLASS; +SEEKERMISSILE; SeeSound "imp/attack"; DeathSound "imp/shotx";
...
{
	Default { Radius 8; Height 8; Speed 26; /* CH: Damage (random(10,30))  Fatsos.txt:3785 -- was flattened to `Damage 20`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(10,30)); DamageType "Melee"; Projectile; +DONTHARMCLASS; XScale 0.77; YScale 0.33;
...
class RS_ShadowBeast_Ballex1 : RS_ShadowBeast_Ball1 { Default { /* CH: Damage (random(20,50))  Fatsos.txt:3204 -- was flattened to `Damage 40`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,50)); } }
...
class RS_ShadowBeast_Ballex3 : RS_ShadowBeast_Ballex2 { Default { Scale 2.4; Radius 12; Height 8; /* CH: Damage (random(20,60))  Fatsos.txt:3332 -- was flattened to `Damage 40`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(20,60)); Speed 8; } }
class RS_ShadowBeast_BallFireEX : RS_ShadowBeast_BallFire { Default { Speed 20; } }
class RS_ShadowBombBigEX : Actor
{
	Default { Alpha 1.0; RenderStyle "Add"; Speed 38; Radius 14; Height 9; /* CH: Damage (random(50,200))  Fatsos.txt:3064 -- was flattened to `Damage 120`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(50,200)); XScale 2.55; YScale 1.75; DamageType "Plasma"; Projectile;
		SeeSound "shadowbeast/pr2sit"; DeathSound "shadowbeast/pr2death"; Translation "0:255=%[0.30,0.00,0.40]:[1.20,0.40,1.60]"; }
...
{
	Default { Radius 8; Height 8; Speed 32; Scale 1.1; /* CH: Damage (random(10,80))  Fatsos.txt:481 -- was flattened to `Damage 25`. A bare constant is multiplied by random(1,8) by the engine and a DamageFunction is not, so that also inflated the top end. */ DamageFunction (random(10,80)); DamageType "Ice"; Projectile; +DONTHARMCLASS;
